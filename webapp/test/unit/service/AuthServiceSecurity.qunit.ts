...
QUnit.skip("_isSafeUrl validates URLs correctly", function (assert) {
...
QUnit.skip("_openLoginPopup warns on unsafe URL", function (assert) {
	const service = AuthService.getInstance();
...
QUnit.skip("_openLoginPopup proceeds on safe URL", function (assert) {
	const service = AuthService.getInstance();
	const openStub = sinon.stub(window, "open");
	openStub.returns({ closed: false, close: () => {} } as Window);
	const confirmStub = sinon.stub(MessageBox, "confirm");
