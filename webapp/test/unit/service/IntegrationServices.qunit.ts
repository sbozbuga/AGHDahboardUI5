...
QUnit.skip("getQueryLog constructs correct URL with parameters", async function (assert) {
...
QUnit.test("getSlowestQueries strictly limits occurrences to top 5", async function (assert) {
	...
	try {
		...
		assert.strictEqual(
			slowest[0].occurrences.length,
			5,
			`Should only have top 5 occurrences, got ${slowest[0].occurrences.length}`
		);
