...
QUnit.test("Should format numbers with thousands separators", function (assert) {
	...
	assert.ok(
		formatted.includes(",") || formatted.includes(".") || formatted.includes(" ") || formatted.includes("\u202f"),
		"Output should contain a separator"
	);
...
QUnit.test("Should format elapsed time to state text", function (assert) {
	assert.strictEqual(
		formatter.formatElapsedStateText(600),
		"Critical (> 500ms)",
		"> 500ms should be Critical (> 500ms)"
	);
	assert.strictEqual(
		formatter.formatElapsedStateText(500),
		"Warning (> 200ms)",
		"500ms should be Warning (> 200ms) (edge case)"
	);
	assert.strictEqual(formatter.formatElapsedStateText(201), "Warning (> 200ms)", "> 200ms should be Warning (> 200ms)");
	assert.strictEqual(
		formatter.formatElapsedStateText(200),
		"Good (< 200ms)",
		"200ms should be Good (< 200ms) (edge case)"
	);
	...
	assert.strictEqual(
		formatter.formatElapsedStateText("600"),
		"Critical (> 500ms)",
		"String '600' should be Critical (> 500ms)"
	);
});
...
QUnit.test("Should format date correctly", function (assert) {
	...
	assert.ok(
		/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{3}$/.test(formatted),
		"Format matches pattern yyyy-MM-dd HH:mm:ss.SSS"
	);
});
