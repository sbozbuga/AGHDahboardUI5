	...
	private async _getStatsFromLogs(period: string): Promise<AdGuardStats> {
		...
		try {
			...
			for (let i = 0; i < topDomainsList.length; i++) {
				topDomains[i] = { name: String(topDomainsList[i][0]), count: topDomainsList[i][1] };
			}
			...
			for (let i = 0; i < topBlockedDomainsList.length; i++) {
				topBlockedDomains[i] = { name: String(topBlockedDomainsList[i][0]), count: topBlockedDomainsList[i][1] };
			}
			...
			for (let i = 0; i < topClientsList.length; i++) {
				const ip = String(topClientsList[i][0]);
			...
			for (let i = 0; i < topFiltersList.length; i++) {
				const id = Number(topFiltersList[i][0]);
	...
	private async _getTopFiltersFromLogs(scanDepth?: number): Promise<StatsEntry[]> {
		...
		try {
			...
			for (const e of data.data) {
				...
				if (e.filterId && e.filterId > 0) {
					filterCounts.set(e.filterId, (filterCounts.get(e.filterId) || 0) + 1);
				}
			}
